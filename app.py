    ...
    def item_scraped(self, item):
        ...
        def format_phone_number(number):
            ...
            if number.startswith("1"):
                country_code = "(+1)"
                main_number = number[1:]
            elif number.startswith("91"):
                country_code = "(+91)"
                main_number = number[2:]
            ...
            if len(main_number) == 10:
                ...
                formatted_number = main_number

            return f"{country_code} {formatted_number}".strip()
        ...
        for number, country in phone_numbers:
            if number and (number.startswith('1') or number.startswith('+1')):
                country = 'US'
            if number and (number.startswith('33') or number.startswith('+33')):
                country = 'FR'
            if number and (number.startswith('98') or number.startswith('99') or number.startswith('97') or number.startswith('91')   or number.startswith('92')  or number.startswith('93')  or number.startswith('95')  or number.startswith('90')  or number.startswith('96')  or number.startswith('97') or number.startswith('+91')):
                country = 'IN'

            if country in ["US", "CA"]:
                us_ca_numbers.append((number, country))
            else:
                other_numbers.append((number, country))
        ...
        # Append US and CA numbers first, then others
        for number, country in us_ca_numbers:
            row_data.append(format_phone_number(number))
            row_data.append(country)
        for number, country in other_numbers:
            row_data.append(format_phone_number(number))
            row_data.append(country)
    ...
    def update_time(self):
        if self.start_time is not None:
            ...
            if progress > 0:
                ...
                remaining_time = total_time - elapsed_time
                self.remaining_label.setText(f"Time remaining: {remaining_time:.2f}s")
                print(f"Time remaining: {remaining_time:.2f}s")
            else:
                self.remaining_label.setText("Time remaining: Calculating...")
